...
def shield(label, message, color):
    """A static shields.io badge URL. shields.io escapes literal '-' as
    '--' and spaces as '%20' within each segment."""
    def enc(s):
        return str(s).replace("-", "--").replace(" ", "%20")
...
def regenerate_readme(vault_dir, cfg):
    ...
    badges = " ".join([
        f"![Solved]({shield('Solved', total, 'blue')})",
        f"![Easy]({shield('Easy', easy, '00b8a3')})",
        f"![Medium]({shield('Medium', medium, 'ffc01e')})",
        f"![Hard]({shield('Hard', hard, 'ff375f')})",
        f"![Streak]({shield('Streak', f'{streak}d', 'orange' if streak else 'lightgrey')})",
        f"![Active Days]({shield('Active Days', active_days, 'brightgreen')})",
    ])

    if streak > 0:
        streak_alert = (f"> [!TIP]\n"
                         f"> On a **{streak}-day streak** -- longest on record is "
                         f"**{longest} day{'s' if longest != 1 else ''}**. Keep it going.")
    else:
        streak_alert = (f"> [!WARNING]\n"
                         f"> No active streak right now. Longest on record: "
...
<div align="center"><sub>Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</sub></div>
"""
