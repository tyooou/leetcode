---
link: https://leetcode.com/problems/find-the-largest-almost-missing-integer/description/
difficulty: Easy
date: 2026-08-19
tags: [array, hash-table]
runtime: "0 ms"
memory: "19.4 MB"
---

### Problem Statement:

You are given an integer array `nums` and an integer `k`.

An integer `x` is **almost missing** from `nums` if `x` appears in _exactly_ one subarray of size `k` within `nums`.

Return the **largest** **almost missing** integer from `nums`. If no such integer exists, return `-1`.

A **subarray** is a contiguous sequence of elements within an array.

> [!example]- Example 1
> **Input:** nums = [3,9,2,1,7], k = 3
>
> **Output:** 7
>
> **Explanation:**
>
> 	- 1 appears in 2 subarrays of size 3: `[9, 2, 1]` and `[2, 1, 7]`.
>
> 	- 2 appears in 3 subarrays of size 3: `[3, 9, 2]`, `[9, 2, 1]`, `[2, 1, 7]`.
>
> 	3 appears in 1 subarray of size 3: `[3, 9, 2]`.
> 	7 appears in 1 subarray of size 3: `[2, 1, 7]`.
> 	9 appears in 2 subarrays of size 3: `[3, 9, 2]`, and `[9, 2, 1]`.
>
> We return 7 since it is the largest integer that appears in exactly one subarray of size `k`.

> [!example]- Example 2
> **Input:** nums = [3,9,7,2,1,7], k = 4
>
> **Output:** 3
>
> **Explanation:**
>
> 	- 1 appears in 2 subarrays of size 4: `[9, 7, 2, 1]`, `[7, 2, 1, 7]`.
>
> 	- 2 appears in 3 subarrays of size 4: `[3, 9, 7, 2]`, `[9, 7, 2, 1]`, `[7, 2, 1, 7]`.
>
> 	- 3 appears in 1 subarray of size 4: `[3, 9, 7, 2]`.
>
> 	- 7 appears in 3 subarrays of size 4: `[3, 9, 7, 2]`, `[9, 7, 2, 1]`, `[7, 2, 1, 7]`.
>
> 	- 9 appears in 2 subarrays of size 4: `[3, 9, 7, 2]`, `[9, 7, 2, 1]`.
>
> We return 3 since it is the largest and only integer that appears in exactly one subarray of size `k`.

> [!example]- Example 3
> **Input:** nums = [0,0], k = 1
>
> **Output:** -1
>
> **Explanation:**
>
> There is no integer that appears in only one subarray of size 1.

> [!warning]- Constraints
> - `1 <= nums.length <= 50`
>
> 	- `0 <= nums[i] <= 50`
>
> 	- `1 <= k <= nums.length`

---

### Hints:

> [!success]- Hint 1
> Solve the problem for three different cases: `k = 1`, `k = n`, and `1 < k < n`

> [!success]- Hint 2
> If `k = 1`, return the largest element that occurs exactly once in `nums`

> [!success]- Hint 3
> If `k = n`, return the largest element in `nums`

> [!success]- Hint 4
> If `1 < k < n`, all elements different from `nums[0]` and `nums[n - 1]` will occur in more than one subarray of size `k`. Hence, the answer is the largest of `nums[0]` and `nums[n - 1]` if they both occur exactly once in the array. If one of them occurs more than once, return the other. If both of them occur more than once, return -1.

---

### Submission:

> [!example]+ Submission
> | Runtime | Memory | Time | Space |
> |---|---|---|---|
> | `0 ms` | `19.4 MB` | `O(n)` | `O(1)` |
>
> ```python
> class Solution:
>     def largestInteger(self, nums: List[int], k: int) -> int:
>         n = len(nums)
>         if n == k:
>             return max(nums)
>         count = [0] * 51
>         for x in nums:
>             count[x] += 1
>         if k == 1:
>             for i in range(50, -1, -1):
>                 if count[i] == 1:
>                     return i
>             return -1
>         res = -1
>         if count[nums[0]] == 1:
>             res = max(res, nums[0])
>         if count[nums[-1]] == 1:
>             res = max(res, nums[-1])
>         return res
> ```

> [!success]- Reasoning
> 1. If `n == k`, return max (all subarrays are the full array, so each element appears exactly once).
> 2. Count element frequencies in a fixed-size array (values 0-50).
> 3. For `k == 1`, find the largest element with count exactly 1.
> 4. For `1 < k < n`, only `nums[0]` and `nums[n-1]` can appear in exactly one k-subarray; return the largest with count 1, or -1 if none.
>
> **Key insight:** interior elements always appear in multiple k-subarray instances; only boundary elements can be "almost missing."

---

### Notes:

*(fill in)*
