---
link: https://leetcode.com/problems/make-lexicographically-smallest-array-by-swapping-elements/description/
difficulty: Medium
date: 2026-08-29
tags: [array, union-find, sorting]
runtime: "195 ms"
memory: "96.4 MB"
---

### Problem Statement:

You are given a **0-indexed** array of **positive** integers `nums` and a **positive** integer `limit`.

In one operation, you can choose any two indices `i` and `j` and swap `nums[i]` and `nums[j]` **if** `|nums[i] - nums[j]| <= limit`.

Return _the **lexicographically smallest array** that can be obtained by performing the operation any number of times_.

An array `a` is lexicographically smaller than an array `b` if in the first position where `a` and `b` differ, array `a` has an element that is less than the corresponding element in `b`. For example, the array `[2,10,3]` is lexicographically smaller than the array `[10,2,3]` because they differ at index `0` and `2 < 10`.

> [!example]- Example 1
> **Input:**
>
> ```python
> nums = [1,5,3,9,8], limit = 2
> ```
>
> **Output:**
>
> ```python
> [1,3,5,8,9]
> ```
>
> **Explanation:**
>
> ```python
> Apply the operation 2 times:
> - Swap nums[1] with nums[2]. The array becomes [1,3,5,9,8]
> - Swap nums[3] with nums[4]. The array becomes [1,3,5,8,9]
> We cannot obtain a lexicographically smaller array by applying any more operations.
> Note that it may be possible to get the same result by doing different operations.
> ```

> [!example]- Example 2
> **Input:**
>
> ```python
> nums = [1,7,6,18,2,1], limit = 3
> ```
>
> **Output:**
>
> ```python
> [1,6,7,18,1,2]
> ```
>
> **Explanation:**
>
> ```python
> Apply the operation 3 times:
> - Swap nums[1] with nums[2]. The array becomes [1,6,7,18,2,1]
> - Swap nums[0] with nums[4]. The array becomes [2,6,7,18,1,1]
> - Swap nums[0] with nums[5]. The array becomes [1,6,7,18,1,2]
> We cannot obtain a lexicographically smaller array by applying any more operations.
> ```

> [!example]- Example 3
> **Input:**
>
> ```python
> nums = [1,7,28,19,10], limit = 3
> ```
>
> **Output:**
>
> ```python
> [1,7,28,19,10]
> ```
>
> **Explanation:**
>
> ```python
> [1,7,28,19,10] is the lexicographically smallest array we can obtain because we cannot apply the operation on any two indices.
> ```

> [!warning]- Constraints
> - `1 <= nums.length <= 10^5`
>
> 	- `1 <= nums[i] <= 10^9`
>
> 	- `1 <= limit <= 10^9`

---

### Hints:

> [!success]- Hint 1
> Construct a virtual graph where all elements in `nums` are nodes and the pairs satisfying the condition have an edge between them.

> [!success]- Hint 2
> Instead of constructing all edges, we only care about the connected components.

> [!success]- Hint 3
> Can we use DSU?

> [!success]- Hint 4
> Sort `nums`. Now we just need to consider if the consecutive elements have an edge to check if they belong to the same connected component. Hence, all connected components become a list of position-consecutive elements after sorting.

> [!success]- Hint 5
> For each index of `nums` from `0` to `nums.length - 1` we can change it to the current minimum value we have in its connected component and remove that value from the connected component.

---

### Submission:

> [!example]+ Submission
> | Runtime | Memory | Time | Space |
> |---|---|---|---|
> | `195 ms` | `96.4 MB` | `O(n log n)` | `O(n)` |
>
> ```python
> class Solution:
>     def lexicographicallySmallestArray(self, nums: List[int], limit: int) -> List[int]:
>         best = sorted(nums)
>         groups = defaultdict(deque)
>         mapping = {}
>
>         prev, idx = best[0], 0
>         for v in best:
>             if v > prev + limit:
>                 idx += 1
>             groups[idx].append(v)
>             mapping[v] = idx
>             prev = v
>
>         return [groups[mapping[v]].popleft() for v in nums]
> ```

> [!success]- Reasoning
> 1. Sort the array to identify which elements can be grouped into connected components based on the limit constraint.
> 2. Iterate through sorted array, grouping consecutive values within limit distance into separate components using a deque.
> 3. Map each value to its component group index to track which pool each element belongs to.
> 4. For each position in the original array, extract the smallest available value from that value's component group.
> 5. Return the result array with elements replaced by optimal smaller values from their component groups.
>
> **Key insight:** Sort to identify swappable groups, then greedily assign smallest available value from each element's group.

---

### Notes:

*(fill in)*
