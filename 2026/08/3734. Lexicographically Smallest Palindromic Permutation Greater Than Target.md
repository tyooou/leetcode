...
> [!success]- Reasoning
> 1. Check if palindrome is possible: at most one character can have odd frequency.
> 2. Greedily construct the left half by trying the smallest available character at each position.
> 3. For each candidate character, build the full palindrome and check if it exceeds target.
> 4. Once the prefix becomes lexicographically greater than target's prefix, fill remaining positions with smallest chars.
> 5. Mirror the left half, insert middle character if needed, and return the palindrome.
>
> **Key insight:** Greedy left-half construction with early termination once prefix > target's prefix saves comparisons.
