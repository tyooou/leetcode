---
link: https://leetcode.com/problems/shortest-and-lexicographically-smallest-beautiful-string/description/
difficulty: Medium
date: 2026-08-26
tags: [string, sliding-window]
runtime: "0 ms"
memory: "19.2 MB"
---

### Problem Statement:

You are given a binary string `s` and a positive integer `k`.

A substring of `s` is **beautiful** if the number of `1`'s in it is exactly `k`.

Let `len` be the length of the **shortest** beautiful substring.

Return _the lexicographically **smallest** beautiful substring of string _`s`_ with length equal to _`len`. If `s` doesn't contain a beautiful substring, return _an **empty** string_.

A string `a` is lexicographically **larger** than a string `b` (of the same length) if in the first position where `a` and `b` differ, `a` has a character strictly larger than the corresponding character in `b`.

	- For example, `"abcd"` is lexicographically larger than `"abcc"` because the first position they differ is at the fourth character, and `d` is greater than `c`.

> [!example]- Example 1
> **Input:**
>
> ```python
> s = "100011001", k = 3
> ```
>
> **Output:**
>
> ```python
> "11001"
> ```
>
> **Explanation:**
>
> ```python
> There are 7 beautiful substrings in this example:
> 1. The substring "100011001".
> 2. The substring "100011001".
> 3. The substring "100011001".
> 4. The substring "100011001".
> 5. The substring "100011001".
> 6. The substring "100011001".
> 7. The substring "100011001".
> The length of the shortest beautiful substring is 5.
> The lexicographically smallest beautiful substring with length 5 is the substring "11001".
> ```

> [!example]- Example 2
> **Input:**
>
> ```python
> s = "1011", k = 2
> ```
>
> **Output:**
>
> ```python
> "11"
> ```
>
> **Explanation:**
>
> ```python
> There are 3 beautiful substrings in this example:
> 1. The substring "1011".
> 2. The substring "1011".
> 3. The substring "1011".
> The length of the shortest beautiful substring is 2.
> The lexicographically smallest beautiful substring with length 2 is the substring "11".
> ```

> [!example]- Example 3
> **Input:**
>
> ```python
> s = "000", k = 1
> ```
>
> **Output:**
>
> ```python
> ""
> ```
>
> **Explanation:**
>
> ```python
> There are no beautiful substrings in this example.
> ```

> [!warning]- Constraints
> - `1 <= s.length <= 100`
>
> 	- `1 <= k <= s.length`

---

### Hints:

> [!success]- Hint 1
> Notice that if we consider that index `i` is the leftmost index of a beautiful substring, it has only one candidate `j`, such that `s[i:j]` is beautiful and shortest too.

> [!success]- Hint 2
> We can iterate over all possibilities of leftmost index `i` take `s[i:j]` and compare with the shortest and the lexicographically smallest beautiful string we could get before index `i`.

---

### Submission:

> [!example]+ Submission
> | Runtime | Memory | Time | Space |
> |---|---|---|---|
> | `0 ms` | `19.2 MB` | `O(n²)` | `O(k)` |
>
> ```python
> class Solution:
>     def shortestBeautifulSubstring(self, s: str, k: int) -> str:
>         idx = [-1] * k
>         s_len = len(s)
>
>         i = p = 0
>         mn = 101
>
>         l, r = -1, -1
>         for i in range(s_len):
>             if s[i] == "1":
>                 idx[p] = i
>                 nxt = (p + 1) % k
>                 if idx[nxt] != -1:
>                     nl, nr = idx[nxt], idx[p]
>                     width = nr - nl
>                     if width <= mn and (l == -1 or int(s[nl:nr + 1], 2) < int(s[l:r + 1], 2)):
>                         mn = width
>                         l, r = nl, nr
>                 p = nxt
>
>         return "" if l == -1 else s[l:r + 1]
>
>
>
>
> ```

> [!success]- Reasoning
> 1. Use a circular array `idx` of size k to track positions of the last k ones in the string.
> 2. Iterate through the string; when a '1' is found, store its index and check if we have k ones (by verifying idx[nxt] != -1).
> 3. Each valid substring spans from idx[nxt] to idx[p], containing exactly k ones.
> 4. Compare candidates by width first, then by binary value (lexicographic order) to find the optimal answer.
>
> **Key insight:** Circular buffer tracks only k one-positions, enabling single-pass identification of all minimal k-one substrings.

---

### Notes:

*(fill in)*
