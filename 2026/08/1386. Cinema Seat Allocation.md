---
link: https://leetcode.com/problems/cinema-seat-allocation/description/
difficulty: Medium
date: 2026-08-20
tags: [array, hash-table, greedy, bit-manipulation]
runtime: "31 ms"
memory: "22.5 MB"
---

### Problem Statement:

A cinema has `n` rows of seats, numbered from 1 to `n`. Each row has 10 seats, numbered from 1 to 10.

You are given a 2D integer array reservedSeats, where reservedSeats[i] = [row_i, seat_i] means that seat seat_i in row row_i is already reserved.

A four-person group must be assigned to four seats in the **same** row. The group can be seated in one of the following seat blocks:

	- seats 2, 3, 4, 5

	- seats 4, 5, 6, 7

	- seats 6, 7, 8, 9

A block can be used only if **none** of its seats are reserved. Each seat can be assigned to **at most **one group.

Return an integer denoting the **maximum** number of four-person groups that can be assigned.

> [!example]- Example 1
> **Input:**
>
> ```python
> n = 3, reservedSeats = [[1,2],[1,3],[1,8],[2,6],[3,1],[3,10]]
> ```
>
> **Output:**
>
> ```python
> 4
> ```
>
> **Explanation:**
>
> ```python
> The figure above shows an optimal allocation of four groups. Seats marked in blue are already reserved, and each set of four contiguous seats marked in orange is assigned to one group.
> ```

> [!example]- Example 2
> **Input:**
>
> ```python
> n = 2, reservedSeats = [[2,1],[1,8],[2,6]]
> ```
>
> **Output:**
>
> ```python
> 2
> ```

> [!example]- Example 3
> **Input:**
>
> ```python
> n = 4, reservedSeats = [[4,3],[1,4],[4,6],[1,7]]
> ```
>
> **Output:**
>
> ```python
> 4
> ```

> [!warning]- Constraints
> - `1 <= n <= 10^9`
>
> 	- `1 <= reservedSeats.length <= min(10 * n, 10^4)`
>
> 	- `reservedSeats[i] == [row_i, seat_i]`
>
> 	- `1 <= row_i <= n`
>
> 	- `1 <= seat_i <= 10`
>
> 	- All `reservedSeats[i]` are distinct.

---

### Hints:

> [!success]- Hint 1
> Note you can allocate at most two four-person groups in one row.

> [!success]- Hint 2
> Greedily check if you can allocate seats for two groups, one group or none.

> [!success]- Hint 3
> Process only rows that appear in the input, for other rows you can always allocate seats for two groups.

---

### Submission:

> [!example]+ Submission
> | Runtime | Memory | Time | Space |
> |---|---|---|---|
> | `31 ms` | `22.5 MB` | `O(m)` | `O(m)` |
>
> ```python
> class Solution:
>     def maxNumberOfFamilies(
>         self, n: int, reservedSeats: List[List[int]]
>     ) -> int:
>         left, middle, right = 0b11110000, 0b11000011, 0b00001111
>         occupied = collections.defaultdict(int)
>         for seat in reservedSeats:
>             if 2 <= seat[1] <= 9:
>                 occupied[seat[0]] |= 1 << (seat[1] - 2)
>
>         ans = (n - len(occupied)) * 2
>         for row, bitmask in occupied.items():
>             if (
>                 (bitmask | left) == left
>                 or (bitmask | middle) == middle
>                 or (bitmask | right) == right
>             ):
>                 ans += 1
>         return ans
> ```

> [!success]- Reasoning
> 1. Each unreserved row can fit exactly 2 groups (seats 2–5 and 6–9, or 4–7 and 8–9 variations).
> 2. Build a bitmask for each reserved row: bits 2–9 encode occupied seats (only bits for seats 2–9 matter).
> 3. For each reserved row, check if any of the three target blocks (2–5, 4–7, 6–9) remain free using bitmask OR operations.
> 4. Count unreserved rows as `(n - num_reserved_rows) * 2`, then add 1 for each reserved row that can still fit a group.
>
> **Key insight:** only rows with reservations in the 2–9 range affect allocation; others always fit 2 groups; bitmasks check block occupancy in O(1).

---

### Notes:

*(fill in)*
